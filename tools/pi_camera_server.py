...
라즈베리파이에서:
    python tools/pi_camera_server.py --source 0 --port 8000
PC의 config.CAMERA_SOURCE:
    "http://raspberrypi.local:8000/"   또는  "http://192.168.x.x:8000/"

의존성: opencv-python 만 있으면 된다(표준 라이브러리 http.server 사용). USB 웹캠 기준이며,
CSI 리본 카메라는 OS 카메라 스택 설정에 따라 인덱스가 다를 수 있다(파이에서 실측 확인).
...
def make_handler(source, jpeg_quality, fps_limit):
...
def main():
    ...
    parser.add_argument("--source", default=0, help="웹캠 인덱스(기본 0) 또는 영상 경로")
    ...
    handler = make_handler(source, args.quality, args.fps)
