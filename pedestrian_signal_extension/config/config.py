...
# 제어부가 정한다. 잔여 녹색 시간을 7세그먼트로 직접 세는 쪽만 "지금 뒤처졌는가"에
# 답할 수 있기 때문이다 (docs/team_interface.md "아두이노 계약").
#
# 값을 여기 남겨 두는 이유는 **근거가 여기 적혀 있기 때문**이다. 스케치에 숫자만 옮겨
# 적으면 왜 그 값인지가 사라진다.
...
#
# 하드코딩하지 말고 위 식으로 계산할 것. BASE_GREEN_SEC이 바뀌면 표 전체가 바뀐다.
...
# 연장은 되돌릴 수 없다. **낭비의 상한이 곧 이 값**이므로 작을수록 차량 대기가 줄어든다.
