...
# 경우만 덮고, "새 번호로 보이는" 경우는 못 덮는다. 그때 잔류 카운트가 0으로 돌아가는
# 것보다 **CrossingProgress의 진입 방향이 날아가는 것**이 훨씬 비싸다:
#
#     정상          물리구역 1 2 3 4 5 -> 진척도 1 2 3 4 5
#     4번에서 재발급  물리구역     4 5 -> 진척도     2 1   <- 방향이 뒤집힌다
#
# 거의 다 건넌 사람이 "방금 진입"으로 보고되고, 진척도가 감소하면서 아두이노가 매번
# 새로 연장한다. 재확정 지연(8fps에서 0.25초)과는 비교가 안 되는 손해다.
#
# 그래서 '처음 보는 ID'가 직전에 사라진 트랙의 발 위치 근처에 나타나면 같은 사람으로 보고
# 상태를 통째로 물려준다. 쓰러짐 감지의 같은 보정(FALL_CONFIG["track_inherit_overlap"])은
# bbox 겹침을 쓰지만, 여기는 발 위치만 받으므로 **거리**로 맞춘다 — 구역 판정과 같은 좌표다.
#
# 0으로 두면 이 보정이 꺼진다(동작을 끄고 비교해볼 때 쓴다).

# 호모그래피가 있을 때(실측 치수가 채워져 있을 때) 쓰는 값 — cm.
#
# ⚠️ 5.0은 **추정치다.** 모형 횡단보도 길이 40cm를 5구역으로 나누면 구역당 8cm이므로
# 그보다 작게 잡아 "옆 구역 사람을 물려받는" 일이 없게 한 것뿐이다. 실제로는 발 위치
# 지터(bbox 하단이 프레임마다 떠는 정도)가 지배적일 텐데, 그건 실측 전에는 알 수 없다.
# tools/manual_camera_person_check.py 로 한 사람을 세워 두고 발 위치가 몇 cm 흔들리는지
# 본 뒤 조정할 것.
#
# 값의 위아래가 모두 막혀 있다:
#
#   하한 — **공백 동안 사람이 움직인 거리보다 커야 한다.** 안 그러면 상속이 아예 안 된다.
#          8fps, 유예 2프레임이면 최대 3프레임(0.375초) 안 보일 수 있고, 모형은 2.5cm/s
#          이므로 약 0.9cm를 움직인다. 여기에 발 위치 지터를 더한 것이 하한이다.
#   상한 — **구역 크기(40cm/5 = 8cm)를 넘으면 안 된다.** 넘으면 옆 구역의 남남을 잇는다.
#
# 5.0은 그 사이에서 넉넉하게 잡은 값이다. 모형이 더 빨라지거나 FPS가 떨어지면 하한이
# 올라가므로 다시 계산할 것.
TRACK_INHERIT_DISTANCE_CM = 5.0

# 실측 치수가 없어 호모그래피가 없을 때 쓰는 값 — 픽셀.
#
# ⚠️ 픽셀 값은 카메라 해상도와 설치 거리에 통째로 의존하므로 **이식성이 없다.**
# 가능하면 CROSSWALK_REAL_WIDTH_CM / LENGTH_CM 을 채워 cm 경로를 쓸 것.
