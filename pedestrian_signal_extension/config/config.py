...
# --- 아래 두 값은 **아두이노(제어부)가 쓰는 값**이다. 파이 코드는 읽지 않는다. ---
#
# 프로토콜을 4상태(NORMAL / EXTEND <초> <ETA> / FALL)로 단순화하면서 "언제 연장할까"의
# 판단이 제어부로 옮겨갔다. 잔여 녹색 시간을 7세그먼트로 직접 세는 쪽만 그 질문에
# 답할 수 있기 때문이다(docs/team_interface.md "아두이노 계약" 참고).
#
# 값을 여기에 남겨 두는 이유는 **근거가 여기 적혀 있기 때문**이다. 아두이노 스케치에
# 숫자만 옮겨 적으면 왜 5초·10초인지가 사라진다. 스케치를 쓰는 사람이 이 주석을 보고
# 같은 값을 넣도록 한다.

# 연장 트리거를 발동할 "남은 신호 시간" 임계값(초). 이 값 이하로 남았을 때만 연장한다.
#
...
#
# ⚠️ 아두이노 구현 시 주의: 파이는 조건이 유지되는 동안 같은 EXTEND를 계속 재전송한다
# (유실 복구용). 받을 때마다 더하면 녹색이 무한히 늘어난다. **하강 구간당 한 번만**
# 적용하는 플래그가 반드시 필요하다 — team_interface.md의 appliedThisDescent 참고.
...
# ⚠️ 체인 대입(`A = B = {...}`)을 쓰지 말 것. 그렇게 쓰면 일반 맵까지 같은 값으로
# 덮어씌워져 **일반 보행자에게도 교통약자 값이 나간다.** 실제로 한 번 그렇게 돼 있었다.
PRIORITY_ZONE_EXTENSION_SEC = {
    1: 0,     # 양 끝 구역 — 연장 안 함 (일반 규칙과 동일)
    2: 4,     # 3 x 1.25 = 3.75 -> 올림. 과소 연장이 과대 연장보다 위험하므로 올림한다.
    3: 6,     # 5 x 1.25 = 6.25 -> 6. 서울시 "3초~최대 6초" 범위의 상한과도 일치한다.
    4: 4,     # 2번과 대칭
    5: 0,     # 양 끝 구역 — 연장 안 함
}
# 근거: 경찰청 보행신호시간 산정이 일반 보행속도 1.0m/s를 보행약자 지역에서 0.8m/s로
# 완화하며, 그 비율 1.0/0.8 = 1.25를 일반 값에 곱했다. ETA_SAFETY_MARGIN과 같은 근거다.
# 3번의 6초는 1.25배 계산과 서울시 사례 상한이 독립적으로 같은 값을 가리킨다.
...
# 계산된 ETA(예상 통과 시간)를 아두이노로 함께 보낼지 여부.
#
# True  -> `EXTEND 5 7.2` 처럼 ETA를 실어 보낸다. 아두이노가 부족분을 계산해 연장을 깎는다.
# False -> `EXTEND 5 -`   처럼 자리를 비워 보낸다. 아두이노는 구역값을 그대로 쓴다.
#
# 뺄셈이 아두이노에서 일어나는 이유: 계산식이
#
#     부족분 = ETA x ETA_SAFETY_MARGIN - 남은 신호 시간
#     연장   = min( 구역값, ceil(부족분) )          # 부족분 <= 0 이면 연장 안 함
#
# 인데 **파이는 남은 신호 시간을 모른다.** 두 숫자를 다 가진 쪽이 계산해야 한다.
#
# 구역값이 상한으로 남는 성질은 그대로다 — 속도는 연장을 '깎기만' 하고 늘리지 않는다.
# 따라서 ETA가 틀려도 연장이 구역값을 넘을 수 없다(안전한 방향의 오차).
#
# 전제: ETA는 호모그래피가 있어야 나온다. CROSSWALK_REAL_*_CM 이 비어 있으면 속도가 px/s로
# 떨어져 ETA가 None이 되고, 그러면 True여도 자동으로 `-`가 나간다(안전한 폴백).
#
...
#
# **이 값도 아두이노가 쓴다** (부족분 계산이 그쪽에서 일어나므로). 근거를 잃지 않도록
# 여기에 남겨 두고, 스케치에 같은 값을 넣는다.
