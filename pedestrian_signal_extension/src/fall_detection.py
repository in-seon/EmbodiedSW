...
이 파일의 판단 로직은 팀원이 작성한 PoC(`crosswalk_poc.py`)에서 **한 글자도 바꾸지 않고**
그대로 옮겨온 것이다. 아래 "원문 그대로" 구간(Person / foot_in_roi ~ FallTracker)은
...
# 판단 파라미터는 config 한 곳에 모아 둔다 (config.FALL_CONFIG).
# 원문 로직이 cfg dict를 받는 형태라 dict 그대로 노출한다.
FALL_CONFIG = config.FALL_CONFIG
