...
- [ ] **제어부/구동부 보드 구성 확정** — 현재 ESP32(제어부) + 아두이노 우노(구동부)는 **잠정안**.
      부저·신호등 LED·7세그먼트는 **한 보드**가 담당하기로 했고, 그래서 시리얼 채널도 하나다.
- [x] **프레이밍 방식 — 개행으로 끝나는 ASCII 한 줄로 확정** (`SERIAL_MESSAGE_FORMAT = "ascii-lines"`),
      `SERIAL_BAUDRATE = 9600`(아두이노 스케치의 `Serial.begin()`과 일치).
      `SERIAL_PORT`는 `None`이면 연결된 포트에서 자동 탐색한다(후보가 둘 이상이면 실패시키고 목록을 보여줌).

      | 파이 → 아두이노 | 아두이노 → 파이 | 상태 |
      |---|---|---|
      | `ALERT` / `STOP` | `OK <명령>` | ✅ 구현 완료 |
      | `PING` | `PONG`, `READY` | ✅ 구현 완료 |
      | — | `TIMEOUT` (워치독이 알람을 껐음) | ✅ 구현 완료 |
      | `EXTEND <초>` | — | ❌ 아래 항목에서 합의 필요 |
      | — | `REMAIN <초>` | ❌ 합의 필요 |
      | — | `CYCLE` | ❌ 합의 필요 |

      확인 도구: `python tools/manual_buzzer_check.py --port <포트>` (카메라·YOLO 없이 통신만 검증)

- [ ] **연장 명령 `EXTEND <초>`의 의미 — 누적인가 절대값인가.**
      `send_extend_signal()`이 이것 때문에 아직 막혀 있다. 잘못 맞추면 연장량이 의도의 몇 배가 된다.
      priority 플래그 전달 방식도 함께 정할 것.
- [ ] **잔여 녹색 시간 `REMAIN <초>` — 전송 주기 합의 필요.** 파이가 요청할 때 보내는 방식이
      아니라 아두이노가 주기적으로 흘려보내는 방식을 전제로 짜여 있다(`SerialComm.poll()`).
