...
| `EXTEND <초> <ETA초\|->` | 확정 보행자가 가운데 구역에 있음. 예: `EXTEND 5 7.2`, `EXTEND 3 -` |
| `FALL` | 쓰러짐 확정 |
| `PING` | 연결 확인 |
...
- **ETA는 변화 판정에 넣지 않는다.** 걷는 동안 매 프레임 변하므로 그것까지 비교하면
  결국 매 프레임 전송이 된다. 대신 보낼 때마다 최신 ETA를 싣는다.
- 종료 시 `NORMAL`을 보내고 닫는다(부저가 계속 울거나 연장이 남지 않도록).

### 아두이노가 구현해야 할 것

- [ ] **`appliedThisDescent` 플래그 — 없으면 녹색이 무한히 늘어난다.**
      파이는 조건이 유지되는 동안 같은 `EXTEND`를 계속 재전송한다(유실 복구용).
      받을 때마다 더하면 무한 연장이다. **하강 구간당 한 번만** 적용해야 한다.

      ```c
      if (greenRemaining < REMAIN_THRESHOLD && pending > 0 && !appliedThisDescent
          && totalExtended + need <= MAX_TOTAL) {
          greenRemaining += need;
          totalExtended  += need;
          appliedThisDescent = true;
      }
      // 새 녹색 시작 시: totalExtended = 0; appliedThisDescent = false;
      ```
