...
이때 5등분은 **화면이 아니라 실제 거리 기준**이다. 사선 구도에서 화면상 균등 분할은
실제로는 전혀 균등하지 않아(먼 쪽 1픽셀 ≫ 가까운 쪽 1픽셀), 그대로 두면 "정중앙"
구역이 실제 한가운데를 벗어난다. 그래서 호모그래피로 평면 좌표(cm)에서 나눈 뒤
픽셀로 되돌린다.
...
교통약자를 **따로 검출하지 않는다.** 구역 기준표가 정상 보행 속도를 담고 있어서, 느린
사람은 기준 대비 지연으로 자동 검출되어 더 연장받는다 — 느린 이유가 무엇이든 결과가 같다.
관련 코드와 설정은 제거했다(`docs/decisions.md` 2026-08-26).
...
- **치수를 꼭 넣는 게 좋다.** 없으면 (a) 속도가 px/s로만 나오고, (b) 구역이 화면 기준으로
  나뉘어 실제 거리로는 균등하지 않다. 사선 구도에서는 이 왜곡이 커서 횡단보도의 진짜
  한가운데가 3번이 아닌 4번 구역에 들어가기도 한다
